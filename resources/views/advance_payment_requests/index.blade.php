...
@section('content')
    ...
    <div class="app-page-title">
        <div class="page-title-wrapper">
            <div class="page-title-heading">
                ...
                <div>
                    <div class="text-primary">Advance Payment Requests Which Are <span class="orange-text">{{$request_statuses[$advance_payment_request_status]}}</span></div>
                    <div class="page-title-subheading">
                        Below is a list of Advance Payment requests.
                    </div>
                </div>
            </div>
        ...
        <div class="card-body">
            <table style="width: 100%;" id="example" class="table table-hover table-striped table-bordered">
                ...
                >
                    ...
                    <td>{{ $request_statuses[$advance_payment_request_status]}}</td>
                    <td>
                        <div class="btn-group dropdown">
                            ...
                            <div tabindex="-1" role="menu" aria-hidden="true" class="dropdown-menu-right rm-pointers dropdown-menu-shadow dropdown-menu-hover-link dropdown-menu" x-placement="bottom-end" style="position: absolute; will-change: transform; top: 0px; left: 0px; transform: translate3d(-825px, 36px, 0px); padding: 0;">
                                ...
                                <button type="button" tabindex="0" class="dropdown-item message-view-btn">
                                    <i class="dropdown-icon lnr-file-empty"> </i><span>View Message</span>
                                </button>

                                <button type="button" tabindex="0" class="dropdown-item quick-view-btn">
                                    <i class="dropdown-icon lnr-file-empty"> </i><span>Quick View</span>
                                </button>

                                <a type="button" tabindex="0" class="dropdown-item" href="{{ url("advance_payment_request/".$advance_payment_request->id) }}" >
                                    <i class="dropdown-icon lnr-book"> </i><span>Detailed View</span>
                                </a>

                                @if(in_array($advance_payment_request->status, $statusesAllowedForEditing))
                                    <button type="button" tabindex="0" class="dropdown-item quick-edit-btn">
                                        <i class="dropdown-icon lnr-pencil"> </i><span>Quick Edit</span>
                                    </button>
                                    <a type="button" tabindex="0" class="dropdown-item" href="{{ url("advance_payment_request_edit/".$advance_payment_request->id) }}" >
                                        <i class="dropdown-icon lnr-file-add"> </i><span>Edit</span>
                                    </a>
                                @endif

                                @if(in_array($advance_payment_request->status, $statusesAllowedForDeleting))
                    ...
                    <td class="deleting-column invisible">
                        @if(in_array($advance_payment_request->status, $statusesAllowedForDeleting))
    ...
    <!-- script for displaying information & editing -->
    <script type="text/javascript">
        let title = "";
        let itemId = "";
        let itemDetails = "";
        let actionButtons = "";
        let viewType = "quick-view"; //There are three options "quick-view" | "quick-view-with-actions" | message-and-actions
        ...
        function getItemDetails(itemId,action){
            let baseUrl = action === "edit" ? "{{ url('advance_payment_request_edit') }}" : "{{ url('advance_payment_request') }}";

            $.ajax({
                url: baseUrl + "/" + itemId + "/" +viewType,
                headers: {
