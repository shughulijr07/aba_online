...
<div style="display: none;">
    ...
    @foreach($statements as $statement)
        ...
        <div  class="statements ml-4 mr-4">
                                                    ...
                                                    <table style="width: 100%; !important;"  class="table table-hover table-striped table-bordered ">
                                                        ...
                                                        @foreach($projects as $project_number=>$project_name)
                                                            <tr>
                                                                ...
                                                                <td class="no-wrap total-single-project total-one-project{{$n}}" id="total--project{{$n}}--{{$project_number}}">0</td>
                                                            </tr>
                                                        @endforeach
                                                        </tbody>
                                                        <tfoot>
...
<div  class="statements ml-4 mr-4">
    ...
    <div class="report overflow-auto pt-0" style="min-width: 18cm; min-height: 29.7cm; margin-left: 1cm;">
        <div style="min-width: 600px">
            <header>
                <div class="row pb-2">
                    <div class="col text-center" >
            ...
            <main>
                ...
                <div class="row pl-3 pr-3" >
                    <div class="col text-center pt-3 pb-2 " style="background-color: #ddd;">
                        <h3>PROJECTS' HOURS RATIO REPORT</h3>
                    </div>
                </div>
                ...
                <div class="row mt-3">
                    <div class="col-12 text-center">
                        <h4>
                            <strong >Year : <span class="text-danger">{{$time_sheet->year}}</span></strong>
                            <strong>Month :
                                <span class="text-danger">
                                @foreach($months as $value => $month_name)
                                        @if($value == $time_sheet->month) {{$month_name}} @endif
                                    @endforeach
                            </span>
                            </strong>
                        </h4>
                        ...
                        <div class="table-responsive">
                            <table style="width: 100%; !important;"  class="table table-hover table-striped table-bordered ">
                                ...
                                <tr class="bg-primary text-white">
                                    ...
                                    <th class="no-wrap text-right">Project Number</th>
                                    <th class="no-wrap text-right">Total (Hrs)</th>
                                    <th class="no-wrap text-right">Total (%)</th>
                                </tr>
                                </thead>
                                <tbody>
                                @foreach($projects as $project_number=>$project_name)
                                    <tr>
                                        <td class="no-wrap">{{$project_name}}</td>
                                        <td class="no-wrap text-right">{{$project_number}}</td>
                                        <td class="no-wrap text-right project--total--hrs" id="project--total--hrs--{{$project_number}}">0</td>
                                        <td class="no-wrap text-right" id="project--total--prc--{{$project_number}}">0</td>
                                    </tr>
                                @endforeach
                                </tbody>
                                <tfoot>
                                <tr class="bg-danger text-white">
                                    <th class="no-wrap">Total</td>
                                    <th class="no-wrap text-right"></td>
                                    <th class="no-wrap text-right" id="grand-total-project-hrs">0</th>
                                    <th class="no-wrap text-right" id="grand-total-project-prc">0</th>
                                </tr>
                                </tfoot>
...
<!-- this script is used to calculate hrs ratio for projects-->
<script type="text/javascript">

    $(function () {

        calculate_hrs_ratio_for_projects();

    });

    function calculate_hrs_ratio_for_projects(){
        var single_project_totals = $(".total-single-project");
        var grand_total_project_hrs = 0;
        var grand_total_project_prc = 0;


        var single_project_total_id;
        var id_parts;
        var project_no;
        var project_total_id;
        var current_project_total_hrs;
        var new_project_total_hrs;

        single_project_totals.each( function(){

            single_project_total_id = $(this).attr('id');
            id_parts = single_project_total_id.split('--');
            project_no = id_parts[2];
            project_total_id = "#project--total--hrs--"+project_no;
            current_project_total_hrs = $(project_total_id).text();
            new_project_total_hrs = +current_project_total_hrs + +$(this).text();

            $(project_total_id).text(new_project_total_hrs);

            grand_total_project_hrs += +$(this).text();
        });

        $('#grand-total-project-hrs').text(grand_total_project_hrs);

        var project_total_hrs = $(".project--total--hrs");
        project_total_hrs.each(function(){

            project_total_id = $(this).attr('id');
            id_parts = project_total_id.split('--');
            project_no = id_parts[3];

            var project_total_prc = ($(this).text()/grand_total_project_hrs)*100;

            $('#project--total--prc--'+project_no).text(project_total_prc.toFixed(0));


            grand_total_project_prc += +project_total_prc;

        });

        $('#grand-total-project-prc').text(grand_total_project_prc);
