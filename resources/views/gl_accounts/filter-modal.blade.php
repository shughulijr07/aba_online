    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="modalLongTitle">GL Accounts Filtering Form <span class="text-primary" id="question-no-label"></span></h5>
                        ...
                        <div class="form-row">
                            <div class="col-md-4">
                                <div class="position-relative form-group">
                                    <label for="number" class="">Activity Number</label>
                                    <input name="number" id="number" type="text" class="form-control filter-input" data-column="8">
                                </div>
                            </div>
                            <div class="col-md-8">
                                <div class="position-relative form-group">
...
<script type="text/javascript">
    ...
    function fill_datatable(filters = {}){
        $('#glAccountsTable').DataTable({
            ...
            serverSide: true,
            ajax: {
                url: "{{ route('gl_accounts.ajaxGetList') }}",
                data:{ filters: filters },
            },
            colReorder: true,
            columns: [
                { data: '#' },
                { data: 'GL Account Number' },
                { data: 'GL Account Name' },
                { data: 'Action' },
            ],
            dom: 'lBfrtip',
            buttons: {
    ...
    $('#filterButton').click(function(){
        ...
        $('#glAccountsTable').DataTable().destroy();
    ...
    function deleteRecord(glAccountId, glAccountNumber) {
        var _token = $("meta[name='csrf-token']").attr("content");
        ajaxDelete(glAccountId, glAccountNumber, _token);
    }


    function ajaxDelete(glAccountId, glAccountNumber, _token) {


        Swal.fire({
            title: 'You are Deleting GL Account ('+glAccountNumber+') \n Are you sure?',
            ...
            if (result.value) {
                ...
                //ajax request
                $.ajax({
                    url: "{{ route('gl_accounts.ajaxDelete') }}",
                    method: 'POST',
                    data: {
                        "id": glAccountId,
                        "_token": _token,
