...
class TravelRequest extends Model
{
    ...
    public function lines(){
        return $this->hasMany(TravelRequestLine::class);
    }
    ...
    public function rejection(){
        return $this->hasOne(TravelRequestReject::class);

    }

    public function returns(){
        return $this->hasMany(TravelRequestReturn::class);

    }

    public static function get_staff_travel_request_by_date($staff_id, $departure_date,$returning_date){

        $travel_request = TravelRequest::where('staff_id', '=', $staff_id)
            ->where('departure_date','=',$departure_date)
            ->where('returning_date','=',$returning_date)
    ...
    public function get_staff_travel_requests_by_status($staff_id,$year,$status){
        ...
        $travel_requests = TravelRequest::where('staff_id', '=', $staff_id)
            ->where('year','=',$year)
    ...
    public static function countTravelRequests(){
        ...
        //for my requests
        $returnedForCorrection = TravelRequest::where('status', '=',  '0')->where('staff_id','=',$employee_id)->get()->count();
        $waitingForSPVApproval = TravelRequest::where('status', '=', '10')->where('staff_id','=',$employee_id)->get()->count();
        $waitingForACCApproval = TravelRequest::where('status', '=', '20')->where('staff_id','=',$employee_id)->get()->count();
        $waitingForFDApproval  = TravelRequest::where('status', '=', '30')->where('staff_id','=',$employee_id)->get()->count();
        $waitingForMDApproval  = TravelRequest::where('status', '=', '40')->where('staff_id','=',$employee_id)->get()->count();
        $approved              = TravelRequest::where('status', '=', '50')->where('staff_id','=',$employee_id)->get()->count();
        $rejected              = TravelRequest::where('status', '=', '99')->where('staff_id','=',$employee_id)->get()->count();
        ...
        if( in_array($current_user_role, [1,2,3,4,9]) ){ //for Super Administrator or MD or HRM
            $returnedForCorrection2 = TravelRequest::where('status', '=', '0')->get()->count();
            $waitingForSPVApproval2 = TravelRequest::where('status', '=', '10')->get()->count();
            $waitingForACCApproval2 = TravelRequest::where('status', '=', '20')->get()->count();
            $waitingForFDApproval2  = TravelRequest::where('status', '=', '30')->get()->count();
            $waitingForMDApproval2  = TravelRequest::where('status', '=', '40')->get()->count();
            $approved2              = TravelRequest::where('status', '=', '50')->get()->count();
            $rejected2              = TravelRequest::where('status', '=', '99')->get()->count();
        }

        if($current_user_role == 5){ // for SPV
            $returnedForCorrection2 = TravelRequest::where('status', '=', '0')->where('responsible_spv','=',$employee_id)->get()->count();
            $waitingForSPVApproval2 = TravelRequest::where('status', '=', '10')->where('responsible_spv','=',$employee_id)->get()->count();
            $waitingForACCApproval2 = TravelRequest::where('status', '=', '20')->where('responsible_spv','=',$employee_id)->get()->count();
            $waitingForFDApproval2  = TravelRequest::where('status', '=', '30')->where('responsible_spv','=',$employee_id)->get()->count();
            $waitingForMDApproval2  = TravelRequest::where('status', '=', '40')->where('responsible_spv','=',$employee_id)->get()->count();
            $approved2              = TravelRequest::where('status', '=', '50')->where('responsible_spv','=',$employee_id)->get()->count();
            $rejected2              = TravelRequest::where('status', '=', '99')->where('responsible_spv','=',$employee_id)->get()->count();
        }
