...
class RetirementRequest extends Model
{
    ...
    public function lines(){
        return $this->hasMany(RetirementRequestLine::class);
    }

    public function approvals(){
        return $this->hasMany(RetirementRequestApproval::class);

    }

    public function rejection(){
        return $this->hasOne(RetirementRequestReject::class);

    }

    public function returns(){
        return $this->hasMany(RetirementRequestReturn::class);

    }

    public static function get_staff_travel_request_by_date($staff_id, $requested_date){

        $travel_request = RetirementRequest::where('staff_id', '=', $staff_id)
    ...
    public function get_staff_travel_requests_by_status($staff_id,$status){

        $travel_info = ['travel_requests' => [], 'total_days'=> 0];

        $travel_requests = RetirementRequest::where('staff_id', '=', $staff_id)
    ...
    public static function countTravelRequests(){
        ...
        //for my requests
        $returnedForCorrection = RetirementRequest::where('status', '=',  '0')->where('staff_id','=',$employee_id)->get()->count();
        $waitingForSPVApproval = RetirementRequest::where('status', '=', '10')->where('staff_id','=',$employee_id)->get()->count();
        $waitingForACCApproval = RetirementRequest::where('status', '=', '20')->where('staff_id','=',$employee_id)->get()->count();
        $waitingForFDApproval  = RetirementRequest::where('status', '=', '30')->where('staff_id','=',$employee_id)->get()->count();
        $waitingForMDApproval  = RetirementRequest::where('status', '=', '40')->where('staff_id','=',$employee_id)->get()->count();
        $approved              = RetirementRequest::where('status', '=', '50')->where('staff_id','=',$employee_id)->get()->count();
        $rejected              = RetirementRequest::where('status', '=', '99')->where('staff_id','=',$employee_id)->get()->count();
        ...
        if( in_array($current_user_role, [1,2,3,4,9]) ){ //for Super Administrator or MD or HRM
            $returnedForCorrection2 = RetirementRequest::where('status', '=', '0')->get()->count();
            $waitingForSPVApproval2 = RetirementRequest::where('status', '=', '10')->get()->count();
            $waitingForACCApproval2 = RetirementRequest::where('status', '=', '20')->get()->count();
            $waitingForFDApproval2  = RetirementRequest::where('status', '=', '30')->get()->count();
            $waitingForMDApproval2  = RetirementRequest::where('status', '=', '40')->get()->count();
            $approved2              = RetirementRequest::where('status', '=', '50')->get()->count();
            $rejected2              = RetirementRequest::where('status', '=', '99')->get()->count();
        }

        if($current_user_role == 5){ // for SPV
            $returnedForCorrection2 = RetirementRequest::where('status', '=', '0')->where('responsible_spv','=',$employee_id)->get()->count();
            $waitingForSPVApproval2 = RetirementRequest::where('status', '=', '10')->where('responsible_spv','=',$employee_id)->get()->count();
            $waitingForACCApproval2 = RetirementRequest::where('status', '=', '20')->where('responsible_spv','=',$employee_id)->get()->count();
            $waitingForFDApproval2  = RetirementRequest::where('status', '=', '30')->where('responsible_spv','=',$employee_id)->get()->count();
            $waitingForMDApproval2  = RetirementRequest::where('status', '=', '40')->where('responsible_spv','=',$employee_id)->get()->count();
            $approved2              = RetirementRequest::where('status', '=', '50')->where('responsible_spv','=',$employee_id)->get()->count();
            $rejected2              = RetirementRequest::where('status', '=', '99')->where('responsible_spv','=',$employee_id)->get()->count();
        }
