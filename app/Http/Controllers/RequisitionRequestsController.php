...
class RequisitionRequestsController extends Controller
{
    ...
    public function index($status)
    {
        ...
        $travel_requests = RequisitionRequest::where('status', '=', $status)->where('staff_id','=',$employee_id)->get();
        ...
        $travel_request_statuses = RequisitionRequest::$travel_request_statuses;
        $travel_request_status   = $status;

        return view('requisition_requests.index',
    ...
    public function adminIndex( $status )
    {
        ...
        if( in_array($current_user_role, [1,2,3,4,9]) ){
            ...
            $travel_requests = RequisitionRequest::where('status', '=', $status)->get();
        }

        if(in_array($current_user_role,[5,9])){ // for SPV
            $travel_requests = RequisitionRequest::where('status', '=', $status)->where('responsible_spv','=',$employee_id)->get();
        ...
        return view('requisition_requests.admin_index',
            compact('travel_requests', 'travel_request_statuses','travel_request_status',
                'model_name', 'controller_name', 'view_type'));
    ...
    public function myTravelRequestsList(Request $request)
    {
        ...
        $query = RequisitionRequest::query();
    ...
    public function staffTravelRequestsIndex()
    {
        ...
        return view('requisition_requests.staff_records_search',
            compact(   'year', 'initial_year', 'all_staff',
                'model_name', 'controller_name', 'view_type'));
    ...
    public function staffTravelRequestsList(Request $request)
    {
        ...
        return view('requisition_requests.staff_records_list',
    ...
    public function create()
    {
        ...
        $travel_request = new RequisitionRequest();
        ...
        return view('requisition_requests.create',
    ...
    public function store(Request $request)
    {
        ...
        $purpose_of_use = $header_data['purpose_of_use'];
        ...
        $travel_request = RequisitionRequest::get_staff_travel_request_by_date($staff_id, $requested_date);
             
       // dd($travel_request);exit()
        ...
        if($travel_request == null){
            ...
            if($request->hasFile('file')){
                ...
                $travel_request->purpose_of_use = $purpose_of_use;
           ...
           // dd($lines);
            ...
            //save travel request lines
            $travel_request_line = new RequisitionRequestLine();
            $travel_request_line->requisition_request_id = $travel_request->id;
            ...
            return redirect('requisition_request/'.$travel_request->id);

        }else{

            $message = 'You have already made another Requisition Request with similar Departure & Return Dates';
    ...
    {
        $travel_request = RequisitionRequest::find($id);
        return view('requisition_requests.file_preview', compact('travel_request'));
    }
    ...
    public function show($id)
    {
        ...
        return view('requisition_requests.show',
    ...
    public function showAdmin($id)
    {
        ...
        return view('requisition_requests.show_admin',
            compact( 'travel_request','supervisors', 'responsible_spv', 'spv_name', 'employee_name',
                'lines','travel_request_statuses', 'supervisors_mode','projects','terms','travel_requests_summary',
    ...
    public function showTravellingStatement($travel_request_id)
    {
        ...
        $travel_request = RequisitionRequest::find($travel_request_id);
        $travel_request_statuses = RequisitionRequest::$travel_request_statuses;
        $travel_requests_summary = $travel_request->get_travel_summary_for_staff($travel_request->staff->id,$travel_request->year);
        ...
        $spv = new Staff();
        $spv_approval = RequisitionRequestApproval::where('requsition_request_id','=',$travel_request_id)->where('level','=','spv')->first();

        $acc = new Staff();
        $acc_approval = RequisitionRequestApproval::where('requsition_request_id','=',$travel_request_id)->where('level','=','acc')->first();

        $fd = new Staff();
        $fd_approval = RequisitionRequestApproval::where('requsition_request_id','=',$travel_request_id)->where('level','=','fd')->first();

        $md = new Staff();
        $md_approval = RequisitionRequestApproval::where('requsition_request_id','=',$travel_request_id)->where('level','=','md')->first();
        ...
        return view('requisition_requests.statement',
    ...
    public function approveSubmittedTravelRequest($travel_request_id,$comments = ''){
        ...
        if( (in_array($current_user_role, [2,3,4,5,9])) && $travel_request_status == 10 ){ // for SPV, HRM,ACC, FD and MD
            ...
            if($travel_request_spv == $current_user_staff_id){ //approve only if the current user is the supervisor assigned to approve the request
                ...
                $approval = new RequisitionRequestApproval();
                $approval->requisition_request_id = $travel_request_id;
                ...
                event( new RequisitionRequestApprovedBySupervisorEvent($travel_request));

                $message = 'Requisition Request Approved successfully.';

                return redirect('requisition_requests_admin/'.$travel_request_id)->with('message',$message);
            }
            else{

                $message = 'You are not authorised to approve this Requisition request.';
        ...
        if($current_user_role == 4 && $travel_request_status == 20){ //for ACC
            ...
            //send email notifications
            event( new RequisitionRequestApprovedByAccountantEvent($travel_request));
        ...
        if($current_user_role == 9 && $travel_request_status == 30){ //for FD
            ...
            //send email notifications
            event( new RequisitionRequestApprovedByFDEvent($travel_request));
        ...
        if($current_user_role == 1){ //for Super Administrator
            ...
            elseif($travel_request_status == 50){ //Approved
                ...
                $message = 'Requisition Request Have Already Been Approved.';

            }

            else{

                $message = 'You are not allowed to approve this Requisition Request.';
            }
            ...
            $message = 'You are not Authorized to approve this Requisition Request.';
    ...
    public function returnTravelRequest(Request $request){
        ...
        if( ( in_array($current_user_role, [2,3,4,5,9])) && $travel_request_status == 10 ){ // for SPV, HRM,ACC, FD and MD
            ...
            if($travel_request_spv == $current_user_staff_id){ //approve only if the current user is the supervisor assigned to approve the request
                ...
                $return = new RequisitionRequestReturn();
                $return->requisition_request_id = $travel_request_id;
                ...
                //send email notifications
                event( new RequisitionRequestReturnedEvent($return_level,$travel_request));

                $message = 'Requisition Request Returned Successfully.';

                return redirect('requisition_requests_admin/'.$travel_request_id)->with('message',$message);
            }
            else{

                $message = 'You are not authorised to Return this Requisition Request.';
        ...
        if($current_user_role == 2 && $travel_request_status == 40){ //for MD
            ...
            $message = 'Requsition Request Returned Successfully.';
        ...
        if($current_user_role == 1){ //for Super Administrator
            ...
            if( in_array($travel_request_status, [10,20,30,40]) ){ //Waiting For Supervisor,ACC,FD or MD Approval
                ...
                $message = 'You are not allowed to Return this Requisition Request.';
            }
    ...
    public function changeSupervisor(Request $request){
        ...
        if (Gate::denies('access',['change_supervisor','edit'])){
            abort(403, 'You Are Not Allowed To Change Supervisor For This Requisition Request');
        }
        ...
        try{
            ...
            //record this change
            $spv_change = new RequisitionRequestChangedSupervisor();
            $spv_change->requisition_request_id = $travel_request_id;
    ...
    public function rejectTravelRequest(Request $request){
        ...
        if( ( in_array($current_user_role, [2,3,4,5,9])) && $travel_request_status == 10 ){ // for SPV, HRM,ACC, FD and MD
            ...
            if($travel_request_spv == $current_user_staff_id){ //reject only if the current user is the supervisor assigned to approve the request
                ...
                //record travel_request approval
                $reject = new RequisitionRequestReject();
                $reject->requisition_request_id = $travel_request_id;
                ...
                //send email notifications
                event( new RequisitionRequestRejectedEvent($reject_level,$travel_request));
                ...
                $message = 'You are not authorised to Reject this Requisition Request.';
        ...
        if($current_user_role == 1){ //for Super Administrator
            ...
            if( in_array($travel_request_status, [10,20,30,40]) ){ //can reject submitted travel request at any status
                ...
                $message = 'You are not allowed to reject this Requisition Request.';
            }
            ...
            $message = 'You are not Authorized to reject this Requisition Request.';
