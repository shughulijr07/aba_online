...
use App\Models\RetirementRequestApproval;
use App\Models\RetirementRequestChangedSupervisor;
use App\Models\RetirementRequestLine;
use App\Models\RetirementRequestReject;
...
class RetirementRequestsController extends Controller
{
    ...
    public function index($status)
    {
        ...
        $travel_request_statuses = RetirementRequest::$travel_request_statuses;
        $travel_request_status   = $status;

        return view('retirement_requests.index',
    ...
    public function adminIndex( $status )
    {
        ...
        if( in_array($current_user_role, [1,2,3,4,9]) ){
            ...
            $travel_requests = RetirementRequest::where('status', '=', $status)->get();
        }

        if(in_array($current_user_role,[5,9])){ // for SPV
            $travel_requests = RetirementRequest::where('status', '=', $status)->where('responsible_spv','=',$employee_id)->get();
        ...
        return view('retirement_requests.admin_index',
            compact('travel_requests', 'travel_request_statuses','travel_request_status',
                'model_name', 'controller_name', 'view_type'));
    ...
    public function adminIndexPayment($status)
    {
        if (Gate::denies('access',['travel_requests','view'])){
            abort(403, 'Access Denied');
        }
        ...
        if( in_array($current_user_role, [1,2,3,4,9]) ){
            //for Super Administrator or SA, MD, HRM, ACC, FD
            $travel_requests = AdvancePaymentRequest::where('status', '=', $status)->get();
            //dd($travel_requests);
        }
        ...
        return view('retirement_requests.admin_index_payment',
            compact('travel_requests', 'travel_request_statuses','travel_request_status',
                'model_name', 'controller_name', 'view_type'));
    ...
    public function myTravelRequestsIndex()
    {
        ...
        return view('retirement_requests.my_records_search',
    ...
    public function myTravelRequestsList(Request $request)
    {
        ...
        return view('retirement_requests.my_records_list',
    ...
    public function staffTravelRequestsIndex()
    {
        ...
        return view('retirement_requests.staff_records_search',
    ...
    public function staffTravelRequestsList(Request $request)
    {
        ...
        return view('retirement_requests.staff_records_list',
    ...
    public function create($id)
    {
        ...
        $travel_request = new RetirementRequest();
        $travel_requests_summary = true;

         $payment_id = $id;
        //$payment_request = PaymentRequest::select('project_code')->where('id','=',$id)->get();
        //dd($payment_request[0]->project_code);
        ...
        return view('retirement_requests.create',
            compact( 'travel_request','payment_id','travel_requests_summary','projects',
                'supervisors', 'responsible_spv','travel_requests_summary','employee_name', 'terms',
    ...
    public function store(Request $request)
    {
        ...
        $requested_date = MyFunctions::convert_date_to_mysql_format($header_data['requested_date']);
        $purpose_of_payment = $header_data['purpose_of_payment'];
        ...
        $travel_request = RetirementRequest::get_staff_travel_request_by_date($staff_id, $requested_date);
        ...
        if($travel_request == null){
            ...
            if($request->hasFile('file')){
                ...
                $travel_request->payment_request_id = $data['payment_id'];
                //$travel_request->year = date('Y');
                $travel_request->requested_date = $requested_date;
                $travel_request->purpose_of_payment = $purpose_of_payment;
            ...
            return redirect('retirement_request/'.$travel_request->id);

        }else{

            $message = 'You have already made another Payment Request with similar Departure & Return Dates';

            return redirect('new_payment_request')->with('message', $message);
    ...
    public function filePreview($id)
    {
        ...
        return view('retirement_requests.file_preview', compact('travel_request'));
    }
    ...
    public function show($id)
    {
        ...
        $project = new Project();
        // $projects = $project->process_active_projects_for_a_month( date('m',strtotime($travel_request->departure_date)));
        ...
        //$travel_requests_summary = $travel_request->get_travel_summary_for_staff($travel_request->staff->id);
        ...
        return view('retirement_requests.show',
    ...
    public function showTravellingStatement($travel_request_id)
    {
        ...
        $travel_request = RetirementRequest::find($travel_request_id);
        $travel_request_statuses = RetirementRequest::$travel_request_statuses;
        $travel_requests_summary = $travel_request->get_travel_summary_for_staff($travel_request->staff->id,$travel_request->year);
        ...
        $spv = new Staff();
        $spv_approval = RetirementRequestApproval::where('travel_request_id','=',$travel_request_id)->where('level','=','spv')->first();

        $acc = new Staff();
        $acc_approval = RetirementRequestApproval::where('travel_request_id','=',$travel_request_id)->where('level','=','acc')->first();

        $fd = new Staff();
        $fd_approval = RetirementRequestApproval::where('travel_request_id','=',$travel_request_id)->where('level','=','fd')->first();

        $md = new Staff();
        $md_approval = RetirementRequestApproval::where('travel_request_id','=',$travel_request_id)->where('level','=','md')->first();
        ...
        return view('payment_requests.statement',
    ...
    public function update($travel_request_id)
    {
        ...
        return redirect('payment_request/'.$travel_request->id);
    ...
    public function approveTravelRequest(Request $request){
        ...
        if (Gate::denies('access',['approve_travel_request','edit'])){
            abort(403, 'You Are Not Allowed To Approve This Payment Request');
        }
    ...
    public function approveSubmittedTravelRequest($travel_request_id,$comments = ''){
        ...
        if( (in_array($current_user_role, [2,3,4,5,9])) && $travel_request_status == 10 ){ // for SPV, HRM,ACC, FD and MD
            ...
            if($travel_request_spv == $current_user_staff_id){ //approve only if the current user is the supervisor assigned to approve the request
                ...
                $approval = new RetirementRequestApproval();
                ...
                event( new PaymentRequestApprovedBySupervisorEvent($travel_request));

                $message = 'Payment Request Approved successfully.';

                return redirect('retirement_requests_admin/'.$travel_request_id)->with('message',$message);
            }
            else{

                $message = 'You are not authorised to approve this Payment request.';
        ...
        if($current_user_role == 2 && $travel_request_status == 40){ //for MD
            ...
            //record travel_request approval
            $approval = new PaymentRequestApproval();
            $approval->travel_request_id = $travel_request_id;
            $approval->level = $approval_level;
            $approval->done_by = $current_user_staff_id;
            $approval->comments = $comments;
        ...
        if($current_user_role == 1){ //for Super Administrator
            ...
            elseif($travel_request_status == 50){ //Approved
                ...
                $message = 'Retiirement Request Have Already Been Approved.';

            }

            else{

                $message = 'You are not allowed to approve this Payment Request.';
            }
    ...
    public function returnTravelRequest(Request $request){
        ...
        if( ( in_array($current_user_role, [2,3,4,5,9])) && $travel_request_status == 10 ){ // for SPV, HRM,ACC, FD and MD
            ...
            if($travel_request_spv == $current_user_staff_id){ //approve only if the current user is the supervisor assigned to approve the request
                ...
                $return = new RetirementRequestReturn();
                ...
                $message = 'Payment Request Returned Successfully.';

                return redirect('retirement_requests_admin/'.$travel_request_id)->with('message',$message);
            }
            else{

                $message = 'You are not authorised to Return this Payment Request.';
        ...
        if($current_user_role == 9 && $travel_request_status == 30){ //for FD
            ...
            $message = 'Retirement Request Returned Successfully.';
        ...
        if($current_user_role == 1){ //for Super Administrator
            ...
            if( in_array($travel_request_status, [10,20,30,40]) ){ //Waiting For Supervisor,ACC,FD or MD Approval
                ...
                $message = 'You are not allowed to Return this Payment Request.';
            }
            ...
            $message = 'You are not Authorized to Return this Payment Request.';
            return redirect('retirement_requests_admin/'.$travel_request_id)->with('message',$message);
    ...
    public function changeSupervisor(Request $request){
        ...
        if (Gate::denies('access',['change_supervisor','edit'])){
            abort(403, 'You Are Not Allowed To Change Supervisor For This Payment Request');
        }
        ...
        try{
            ...
            //record this change
            $spv_change = new PaymentRequestChangedSupervisor();
    ...
    public function rejectTravelRequest(Request $request){
        ...
        if (Gate::denies('access',['reject_travel_request','edit'])){
            abort(403, 'You Are Not Allowed To Reject This Payment Request');
        }
        ...
        if( ( in_array($current_user_role, [2,3,4,5,9])) && $travel_request_status == 10 ){ // for SPV, HRM,ACC, FD and MD
            ...
            if($travel_request_spv == $current_user_staff_id){ //reject only if the current user is the supervisor assigned to approve the request
                ...
                //record travel_request approval
                $reject = new RetirementRequestReject();
                ...
                $message = 'You are not authorised to Reject this Payment Request.';
        ...
        if($current_user_role == 4 && $travel_request_status == 20){ //for ACC
            ...
            $message = 'Retirement Request Rejected Successfully.';

            return redirect('payment_requests_admin/'.$travel_request_id)->with('message',$message);
        ...
        if($current_user_role == 1){ //for Super Administrator
            ...
            if( in_array($travel_request_status, [10,20,30,40]) ){ //can reject submitted travel request at any status
                ...
                $message = 'You are not allowed to reject this Payment Request.';
            }
            ...
            $message = 'You are not Authorized to reject this Payment Request.';
