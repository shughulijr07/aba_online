...
use App\Mail\LeaveRequestReceived;
...
class LeaveRequestReceivedListner implements ShouldQueue
{
    /**
     ...
     * Handle the event.
     *
     * @param  LeaveRequestReceivedEvent  $event
     * @return void
     */
    public function handle(LeaveRequestReceivedEvent $event)
    {
        $leave = $event->leave;
        ...
        $recipient = $staff;
        Mail::to($employee_official_email)->send(new LeaveRequestReceived($recipient_type,$recipient,$leave));
        sleep(10);
        ...
        Mail::to($spv_official_email)->send(new LeaveRequestReceived($recipient_type,$recipient,$leave));
        sleep(10);
        ...
        if( count($users)>0 ){
            foreach ($users as $user){
                ...
                $recipient_type = 'hrm';
                $recipient = $hrm;
                Mail::to($hrm_official_email)->send(new LeaveRequestReceived($recipient_type,$recipient,$leave));
                sleep(10);
